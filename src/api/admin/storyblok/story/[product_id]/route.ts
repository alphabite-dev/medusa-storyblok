...
import {
  createSbProductWorkflow,
  createSbProductVariantsWorkflow,
  forceSyncSbProductWorkflow,
  forceSyncSbProductVariantsWorkflow,
} from "../../../../../workflows";
...
export const PUT = async (req: MedusaRequest<{ product_id: string }>, res: MedusaResponse) => {
  const container = req.scope;
  const logger = container.resolve(ContainerRegistrationKeys.LOGGER);
  const sbModuleService = container.resolve<StoryblokModuleService>(STORYBLOK_MODULE);
  const query = container.resolve(ContainerRegistrationKeys.QUERY);
  ...
  try {
    // Force sync product story
    await forceSyncSbProductWorkflow.run({ container, input: { product_id } });
    ...
    // Force sync variants if they exist
    if (product[0]?.variants && product[0].variants.length > 0) {
    ...
    const productStory = await sbModuleService.retrieveProductStory({ productId: product_id });
    return res
      .status(200)
      .json({ storyblok_editor_url: sbModuleService.getSbStoryEditorUrl({ id: productStory.id }) });
  } catch (err) {
    logger.error("🔥 Failed to force sync product story", err);
    return res.status(500).end();
  }
};
